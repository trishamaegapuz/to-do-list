  ...
  const handleEdit = async (e, id, oldTitle) => {
    ...
    if (newTitle && newTitle !== oldTitle) {
      try {
        ...
        fetchLists(); // Refresh agad ang UI
      } catch (err) {
        if (err.response?.status === 401) navigate('/');
      }
    }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0f172a] text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      <main className="flex-grow p-6 pb-32">
        <header className="max-w-2xl mx-auto mb-12 mt-4 text-center">
          <div className="flex justify-start mb-10">
            <button onClick={handleLogout} className="px-4 py-2 bg-slate-800/40 hover:bg-red-500/10 hover:text-red-400 border border-slate-700 rounded-xl text-slate-500 text-[10px] font-bold uppercase tracking-widest transition-all">
              ← Logout Session
            </button>
          </div>
          ...
          <p className="text-slate-500 font-medium italic text-sm mt-2">Organize your daily workflow with precision</p>
        </header>
            ...
            lists.map((l) => (
              ...
              >
                <div className="flex flex-col gap-1">
                  <span className="text-xl font-bold text-white group-hover:text-indigo-400 transition-colors tracking-tight">{l.title}</span>
